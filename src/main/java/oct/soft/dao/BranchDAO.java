...
public class BranchDAO {
	...
	public List<Office> branchList() {
		...
		try {
			...
			branches = queryRunner.query("SELECT * FROM office where isbranch=? and parent=? ORDER BY name",
					beanListHandler, 1, 0);
		} catch (Exception ex) {
			ex.printStackTrace();
		}
	...
	public Office saveOrUpdate(Office branch) {
		Office changedBranch = null;
		try {
			if (branch.isNew()) {
				
				sql = "INSERT INTO office(isbranch, name, parent) VALUES (1,?,0);";			
				ScalarHandler<Long> scalarHandler = new ScalarHandler<Long>();
				Integer id = queryRunner.insert(sql, scalarHandler, branch.getName()).intValue();
				changedBranch = getBranch(id);
			} else {
				// update data
				queryRunner.update("UPDATE office set name =? WHERE idoffice = ?;", branch.getName(),
						branch.getIdoffice());
				changedBranch = getBranch(branch.getIdoffice());
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public List<Office> getOfficesByBranch(int branchId) {
		...
		try {
			...
			offices = queryRunner.query("SELECT * FROM office where isbranch=? and parent=? ORDER BY name",
					beanListHandler, 0, branchId);
		} catch (Exception ex) {
			ex.printStackTrace();
		}
		return offices;
